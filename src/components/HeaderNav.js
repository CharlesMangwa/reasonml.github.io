...
export default class HeaderNav extends React.Component {
  constructor(props) {
    ...
    this.state = {
      isToggleOn: false,
      isPageHeaderGray: null, 
    }
    this.toggleMenu = this.toggleMenu.bind(this)
  }

  componentDidMount() {
    this.setState(() => ({ isPageHeaderGray: window.location.pathname.match(/^\/$/) }))
  }
    ...
    if (this.state.isToggleOn) {
      ...
      hamburgerButton = (
        <div css={styles.hamburger} onClick={this.toggleMenu}>
          <span css={[styles.hamburgerBar, styles.hamburgerBarX1, !this.state.isPageHeaderGray && { background: gray }]} />
          <span css={[styles.hamburgerBar, styles.hamburgerBarX2, !this.state.isPageHeaderGray && { background: gray }]} />
        </div>
      )
    } else {
      hamburgerButton = (
        <div css={styles.hamburger} onClick={this.toggleMenu}>
          <span css={[styles.hamburgerBar, !this.state.isPageHeaderGray && { background: gray }]} />
          <span css={[styles.hamburgerBar, !this.state.isPageHeaderGray && { background: gray }]} />
          <span css={[styles.hamburgerBar, !this.state.isPageHeaderGray && { background: gray }]} />
        </div>
      )
